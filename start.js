...
if (!process.argv[2]) {
	...
	.then((row) => {
		console.log('row', row["dataValues"]["datetime"]);
		const start_at = new Date(row["dataValues"]["datetime"]).getTime() / 1000;
		let end_at = new Date()
		end_at.setMilliseconds(0);
		end_at = end_at.getTime() / 1000;
...
function start(start_at, end_at) {
	const start_day = new Date(start_at);
	// initialize logging
	const logdir = process.env.LOGDIR || null;
	if (logdir != null) {
		if (!fs.existsSync(logdir)) { fs.mkdirSync(logdir); }
		const logDateYmd = formatDateYmd(new Date());
	...
	logger(`Start At: ${new Date(start_at*1000).toUTCString()} (${start_at})`);
